...
public class DispensationHfAdapter extends RecyclerView.Adapter<DispensationHfAdapter.Mholder> {
    private List<RoomPreviousSamplesHf> ParentData = new ArrayList<>();
    private Context context;
    int prevPos = -1;

    public DispensationHfAdapter(List<RoomPreviousSamplesHf> ParentData, Context context) {
    ...
    public void onBindViewHolder(@NonNull Mholder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.date.setText(ParentData.get(position).getD_issue());
    ...
    public void updateList(ArrayList<RoomPreviousSamplesHf> list) {
        ParentData = list;
